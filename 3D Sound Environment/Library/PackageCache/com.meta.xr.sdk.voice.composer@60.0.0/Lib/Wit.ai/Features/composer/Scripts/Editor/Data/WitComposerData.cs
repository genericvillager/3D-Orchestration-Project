...
using System;
using Lib.Wit.Runtime.Data.Info;
using Meta.Voice.Composer.Data;
...
namespace Meta.WitAi.Composer.Data.Info
{
    ...
    public class WitComposerData : WitConfigurationAssetData
    {
        ...
        public override void Refresh(IWitRequestConfiguration configuration)
        {
            WitExportRetriever.GetExport(configuration,
                (exportZip, error) =>
                {
                    ...
                    {
                        canvases = new ComposerParser().ExtractComposerInfo(exportZip).canvases;
                    }
                    else
                    {
