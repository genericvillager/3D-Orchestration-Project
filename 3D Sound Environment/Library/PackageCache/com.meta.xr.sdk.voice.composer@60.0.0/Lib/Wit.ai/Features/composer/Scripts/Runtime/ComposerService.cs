...
namespace Meta.WitAi.Composer
{
    public abstract class ComposerService : MonoBehaviour
    {
        ...
        // Set context map and send an event
        ...
        {
            // Wait a frame, then check for hold
            request.OnHold = true;
            while (request.OnHold)
            {
                yield return new WaitForEndOfFrame();
                request.OnHold = IsSessionActive(sessionId);
            }
